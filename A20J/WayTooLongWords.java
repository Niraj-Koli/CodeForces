...
public class WayTooLongWords {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            StringBuilder s = new StringBuilder(io.next());

            int len = s.length();

            StringBuilder abbrevation = new StringBuilder();

            if (len > 10) {
                abbrevation.append(s.charAt(0));
                abbrevation.append(len - 2);
                abbrevation.append(s.charAt(len - 1));
            } else {
                abbrevation.append(s);
            }

            result[i] = abbrevation.toString();
        }
